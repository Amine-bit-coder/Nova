...
for($i=0;$i<sizeof($tab);$i++){if(isset($tab[$i])){$Id_process=$tab[$i];}}

$tab=$_POST["carte_mere"];
for($i=0;$i<sizeof($tab);$i++){if(isset($tab[$i])){$Id_carte_mere=$tab[$i];}}

$tab=$_POST["RAM"];
for($i=0;$i<sizeof($tab);$i++){if(isset($tab[$i])){$Id_RAM=$tab[$i];}}

$tab=$_POST["disque_dur"];
for($i=0;$i<sizeof($tab);$i++){if(isset($tab[$i])){$Id_disque=$tab[$i];}}

$tab=$_POST["carte_graphique"];
for($i=0;$i<sizeof($tab);$i++){if(isset($tab[$i])){$Id_gpu=$tab[$i];}}
